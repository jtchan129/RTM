...
class Game:
    ...
    def run_night(self, preview_only=True, custom_public_result=None):
        ...
        self.state_df["Name"] = self.state_df["Name"].fillna("").astype(str).str.strip()
        ...
        self.actions_df["Name"] = (
            self.actions_df["Name"].fillna("").astype(str).str.strip()
        )
        self.actions_df["Who do you want to target with your night action"] = (
            self.actions_df["Who do you want to target with your night action"]
            .fillna("")
            .astype(str)
            .str.strip()
        )
        self.actions_df["Who do you want your second target to be"] = (
            self.actions_df["Who do you want your second target to be"]
            .fillna("")
            .astype(str)
            .str.strip()
    ...
    def run_voting(self, preview_only=True, custom_public_result=None):
        ...
        for index, row in self.voting_df.iterrows():
            voting_player_alive = (
                row["Voting Player"]
                in self.state_df.loc[
                    self.state_df["Time died"] == "Alive", "Name"
                ].values
            )
            target_player_alive = (
                row[day_column_name]
            ...
            ) or row[day_column_name] == "No vote"
        ...
        for _, row in valid_votes_df.iterrows():
            if (
                self.state_df.loc[
                    self.state_df["Name"] == row["Voting Player"], "Revealed Mayor"
                ].values
                == 1
            ):
                voting_dict[row[day_column_name]] = (
                    voting_dict[row[day_column_name]] + 3
                )
            else:
                voting_dict[row[day_column_name]] = (
                    voting_dict[row[day_column_name]] + 1
                )
        ...
        if vote_counts[0] == 0:
            ...
            if most_voted == "No vote":
                ...
                most_voted_role = self.state_df.loc[
                    self.state_df["Name"] == most_voted, "Role"
                ].values[0]
                self.public_result = (
                    "On day "
                    + str(day_num)
                ...
                if not preview_only:
                    ...
                    if self.state_df.loc[player_index, "Role"].values[0] == "Jester":
                        ...
                        vote_list = self.voting_df[
                            (self.voting_df[day_column_name] == most_voted)
                            & (self.voting_df["Voting Player"] != jester_name.values[0])
                        ]["Voting Player"].tolist()
                        if vote_list:
                            jester_target_name = random.choice(vote_list)
                            jester_target_index = self.state_df[
                                self.state_df["Name"] == jester_target_name
                            ].index
    ...
    def reveal_mayor(self, mayor_name):
        ...
        if self.state_df.loc[player_index, "Role"].values[0] == "Mayor":
            ...
            send_email(
                self.rtm_group_email,
                mayor_name + " has revealed themselves as mayor",
                "Mayor reveal",
            )
        ...
        for _, row in self.actions_df.iterrows():
            for player in self.player_dict:
                ...
                for target in self.player_dict:
                    if normalize_name(player.get_name()) == normalize_name(
                        row["Name"]
                    ) and normalize_name(target.get_name()) == normalize_name(
                        row["Who do you want to target with your night action"]
                    ):
            ...
            for player in self.player_dict:
                for target in self.player_dict:
                    if normalize_name(player.get_name()) == normalize_name(
                        ...
                        row["Who do you want your second target to be"]
                    ):
        ...
        for player in self.player_dict:
            if player.died_tonight == True:
                if preview_only == False:
                    player_index = self.state_df[
                        self.state_df["Name"] == player.get_name()
                    ].index
