...
class Seducer:
    ...
    @check_target
    def perform_action(self):
        if not self.get_target().roleblock_immune:
            self.get_target().remove_target()
            self.add_result("You seduced " + self.get_target().get_name())
            self.get_target().add_result(
                "You were seduced by an Escort or Hooker and forgot to perform your action"
            )
        else:
            self.add_result(
                "You attempted to seduce "
...
class Follower:
    ...
    def perform_action(self):
        if (
            self.get_target().get_target() is not None
            and str(type(self.get_target()).__name__) != "Amnesiac"
        ):
            self.add_result(
                "You followed "
                + self.get_target().get_name()
                + " to "
                + self.get_target().get_target().get_name()
            )
        else:
            self.add_result(
                "You followed " + self.get_target().get_name() + " to no one"
            )
...
class Watch:
    ...
    def perform_action(self):
        temp_targeted_by = [
            player
            for player in self.get_target().targeted_by()
            if player != self and player != self.get_target()
        ]
        if temp_targeted_by:
            message = (
                "You watched "
            ...
            self.add_result(
                "You watched "
                + self.get_target().get_name()
                + " and they were visited by no one"
            )
    ...
    def perform_action(self):
        if self.get_target().is_guilty:
            self.add_result(
                "You investigated "
                + self.get_target().get_name()
                + " and found they were guilty"
            )
        else:
            self.add_result(
                "You investigated "
                + self.get_target().get_name()
                + " and found they were innocent"
            )
...
class Doctor(Town):
    ...
    def end_action(self):
        temp_attack_1 = [
            player
            for player in self.get_target().attacked_by
            if player.attack_level == 1
        ]
        if temp_attack_1:
            ...
            self.add_result(
                "You protected "
                + self.get_target().get_name()
                + " but did not save them from anything"
            )
    ...
    def perform_action(self):
        if self.get_target().is_lethal:
            self.add_result(
                "You scrutinized "
                + self.get_target().get_name()
                + " and found they were lethal"
            )
        else:
            self.add_result(
                "You scrutinized "
                + self.get_target().get_name()
                + " and found they were non-lethal"
            )
...
class Janitor(Mafia):
    ...
    @check_target_no_increment
    def end_action(self):
        if self.get_target().died_tonight:
            self.add_result(
                "You cleaned "
                + self.get_target().get_name()
                + " and their role was "
                + str(type(self.get_target()).__name__)
            )
        else:
            self.add_result(
                "You attempted to clean "
                + self.get_target().get_name()
...
class Yakuza(Mafia):
    ...
    def end_action(self):
        if issubclass(type(self.get_target()), Mafia):
            self.add_result(
                "You tried to corrupt "
                + self.get_target().get_name()
                + " but they are part of the mafia"
            )
            ...
            self.get_target().add_result(
                "A Yakuza attempted to corrupt you, but you were protected"
            )
...
class Mass_murderer(Neutral):
    ...
    def perform_action(self):
        ...
        temp_targeted_by = [
            player for player in self.get_target().targeted_by() if player != self
        ]
        if temp_targeted_by:
            for i in range(len(temp_targeted_by)):
                self.attack(temp_targeted_by[i])
        if self.get_target().get_target() is None:
            self.attack(self.get_target())
...
class Witch(Neutral, Two_targeter):
    ...
    @check_target2
    def perform_action(self):
        if not self.get_target().control_immune:
            self.get_target().remove_target()
            ...
            self.add_result(
                ...
                + self.get_target().get_name()
                + " to "
                + self.get_target2().get_name()
            )
            self.get_target().add_result("You were controlled")
        else:
            self.add_result(
                "You attempted to control "
                + self.get_target().get_name()
                + " but they were immune"
            )
            self.get_target().add_result(
                "Someone attempted to control you but you are immune"
            )
...
class Amnesiac(Neutral):
    ...
    def perform_action(self):
        if (
            ...
            or not self.get_target().dead
            or str(type(self.get_target()).__name__) == "Godfather"
        ):
            return
